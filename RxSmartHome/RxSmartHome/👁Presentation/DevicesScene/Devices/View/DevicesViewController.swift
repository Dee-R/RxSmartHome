...
class DevicesViewController: UIViewController {
    ...
    var showDeviceDetail: (IndexPath) -> Void = { _ in } /// flow rooter
    ...
    private func bindindRx() {
        ...
        viewModel.dataFilter.bind(to: filtersCV.rx.items(cellIdentifier: FiltersCVCell.reuseID, cellType: FiltersCVCell.self)) { _, value, cell in
            cell.filterLabel.text = "\(value)"

        }.disposed(by: bag) // -- filters --
        viewModel.dataDevices.bind(to: devicesCV.rx.items(cellIdentifier: DevicesCVCell.reuseID, cellType: DevicesCVCell.self)) { _, value, cell in
            cell.deviceTitle.text = "\(value)"
        }.disposed(by: bag) // -- devices --
        filtersCV.rx.itemSelected.asObservable().subscribe(onNext: { [weak self] _ in
    ...
    private func filterDevices(_ typeDevices: String) throws {
